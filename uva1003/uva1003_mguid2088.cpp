...
void process_forest(int case_num, int x_min, int y_min, int x_max, int y_max, const std::vector<tree> &trees) {
  ...
  while(std::accumulate(remaining_trees.begin(), remaining_trees.end(), 0)) {
    bool progress = false;
    for(int i = 0; i < trees.size(); i++) {
      if(remaining_trees[i] == 0) continue;
      auto &t = trees[i];
      ...
      for(int j = 0; j < trees.size(); j++) {
        if( j == i || remaining_trees[j] == 0) continue;
        auto &t2 = trees[j];
  ...
  printf("Forest %d\n%lu tree(s) can be cut\n\n", case_num, trees.size() - std::accumulate(remaining_trees.begin(), remaining_trees.end(), 0));
}
